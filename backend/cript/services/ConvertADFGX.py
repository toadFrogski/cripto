...
class ConvertADFGX(BaseConnvert):
    ...
    def convert_from_ADFG_X(message, key, char_set='adfgx', alphabet='abcdefghiklmnopqrstuvwxyz', options={}):
        ...
        for char in alphabet:
            _alphabet.append(
                {'char': char, 'code': char_set[row] + char_set[index]})
            index = index + 1 if index < len(char_set)-1 else 0
            row = row + 1 if index == 0 else row

        _message = [{'key': keychar, 'tail': [], 'num': index}
        ...
        long_chuncks_nums = [i for i in range(
            len(key) - (long_chunk_length * len(key) - len(message)))]
        ...
        result = []
        for keychar in key:
            item = filter(lambda x: x['tail']
                          and x['key'] == keychar, _message)
        print(_message)
